...
const colorMode = ref<'monochrome' | 'color'>('monochrome')
...
const t = computed(() => ({
  vi: {
    title: 'ASCII',
    subtitle: 'STUDIO',
    description: 'Biến đổi pixel thành dữ liệu ký tự thô. Một sự tôn vinh cho nghệ thuật kỹ thuật số và phong cách J2TEAM.',
    back: 'Quay lại',
    controls: 'THÔNG SỐ',
  ...
  en: {
    title: 'ASCII',
    subtitle: 'STUDIO',
    description: 'Transform pixels into raw character data. A tribute to digital art and the J2TEAM vibe.',
    back: 'Back',
    controls: 'PARAMETERS',
...
const handleFile = (file: File) => {
  if (!file.type.startsWith('image/')) {
    error.value = lang.value === 'vi' ? 'Vui lòng chọn một tệp hình ảnh!' : 'Please select an image file!'
    return
  }
...
<template>
  <div class="min-h-screen bg-bg-deep text-text-primary font-body overflow-x-hidden selection:bg-accent-coral selection:text-bg-deep">
    <!-- Top Decorative Line -->
    <div class="h-1 bg-accent-coral animate-pulse-border"></div>

    <main class="max-w-7xl mx-auto px-6 py-12">
      <!-- Header Section -->
      <header class="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8 animate-fade-up">
        <div class="space-y-4">
          <div class="flex items-center gap-4">
            <div class="inline-block bg-accent-coral text-bg-deep font-display font-bold text-xs tracking-widest px-3 py-1.5 rotate-2">
          ...
          <h1 class="font-display text-6xl md:text-8xl font-bold text-text-primary uppercase tracking-tighter leading-none">
            {{ t.title }}<br/><span class="text-accent-coral">{{ t.subtitle }}</span>
          </h1>
          <p class="text-text-secondary max-w-xl text-lg font-medium border-l-2 border-accent-amber pl-4">
            {{ t.description }}
          </p>
        </div>
        
        <RouterLink to="/" class="group flex items-center gap-3 font-display uppercase tracking-widest text-sm hover:text-accent-coral transition-colors">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <aside class="lg:col-span-4 space-y-8 animate-fade-up animate-delay-1">
          <section class="border border-border-default bg-bg-surface p-8 relative overflow-hidden">
            <span class="absolute top-4 right-4 font-display text-4xl font-bold text-accent-amber/5 select-none pointer-events-none">01</span>
            ...
            <div 
              ...
              :class="[isDragging ? 'border-accent-coral bg-accent-coral/5 scale-[0.98]' : 'hover:border-accent-amber hover:bg-bg-elevated']"
            ...
            >
              ...
              <Icon icon="lucide:upload-cloud" class="text-4xl mb-4 text-text-dim group-hover:text-accent-coral transition-colors" />
              <p v-if="!sourceImage" class="text-text-secondary text-sm">
                {{ t.dropzone }} <span class="text-accent-sky underline">{{ t.browse }}</span>
              </p>
              <p v-else class="text-accent-amber text-sm font-bold truncate max-w-full">{{ t.ready }}</p>
            </div>
            ...
            <div class="space-y-6">
              <div class="space-y-3">
                ...
                <input type="range" v-model.number="resolution" min="20" max="300" step="5"
              ...
              <div class="space-y-3">
                ...
                <input type="range" v-model.number="contrast" min="0.5" max="3.0" step="0.1"
              ...
              <div class="space-y-3">
                <label class="text-xs font-display tracking-widest uppercase text-text-dim">{{ t.charset }}</label>
                <input type="text" v-model="customCharset" 
          ...
          <div class="bg-accent-coral/5 border-l-4 border-accent-coral p-6 space-y-2 opacity-80">
            <h4 class="font-display text-sm font-bold text-accent-coral tracking-widest uppercase">Geek Tip:</h4>
        ...
        <section class="lg:col-span-8 animate-fade-up animate-delay-2">
          <div class="border border-border-default bg-bg-surface h-full min-h-[500px] flex flex-col relative overflow-hidden">
            <!-- Terminal Header -->
            <div class="border-b border-border-default px-6 py-4 flex items-center justify-between bg-bg-deep">
            ...
            <div class="flex-1 overflow-auto p-8 flex flex-col items-center justify-center">
              <!-- Empty State -->
              <div v-if="!sourceImage" class="flex flex-col items-center justify-center space-y-8 animate-pulse">
                <div class="relative flex items-center justify-center">
                  ...
                  <div class="absolute w-24 h-24 border border-accent-coral/20 -rotate-45 animate-spin duration-[6s] linear"></div>
                  <!-- Static Icon -->
                  <Icon icon="lucide:image" class="absolute text-5xl text-accent-amber/20" />
                </div>
                <div class="space-y-2 text-center">
                  <p class="font-display uppercase tracking-[0.4em] text-accent-coral/60 text-xs font-bold">{{ t.awaiting }}</p>
              ...
              <template v-else>
                ...
                <div class="absolute bottom-6 right-6 flex flex-col gap-3 animate-fade-up">
                  <button @click="monoColor = '#FF6B4A'" class="w-6 h-6 bg-[#FF6B4A] border border-white/10 hover:scale-110 transition-transform shadow-lg shadow-[#FF6B4A]/20" />
                  <button @click="monoColor = '#10b981'" class="w-6 h-6 bg-[#10b981] border border-white/10 hover:scale-110 transition-transform shadow-lg shadow-[#10b981]/20" />
                  <button @click="monoColor = '#facc15'" class="w-6 h-6 bg-[#facc15] border border-white/10 hover:scale-110 transition-transform shadow-lg shadow-[#facc15]/20" />
                  <button @click="monoColor = '#38bdf8'" class="w-6 h-6 bg-[#38bdf8] border border-white/10 hover:scale-110 transition-transform shadow-lg shadow-[#38bdf8]/20" />
                  <button @click="monoColor = '#F0EDE6'" class="w-6 h-6 bg-[#F0EDE6] border border-white/10 hover:scale-110 transition-transform shadow-lg shadow-[#F0EDE6]/20" />
                </div>
              </template>
            </div>
          </div>
      ...
      <!-- Footer Branding -->
      <footer class="mt-20 border-t border-border-default pt-8 pb-12 flex flex-col items-center gap-6 animate-fade-up animate-delay-3">
        <div class="font-display text-4xl font-extrabold text-bg-surface select-none pointer-events-none">
          J2TEAM VIBE 2026
        </div>
